...
test("revocable sessions, live account state and scoped service identity fail closed", async () => {
  ...
  try {
    ...
    assert.equal((await request("/api/protected", "fake-token")).status, 401, "fake token");
    const expired = jwt.sign({ id: "operator", sid: "missing", epoch: 0 }, secret, { expiresIn: -1 });
    assert.equal((await request("/api/protected", expired)).status, 401, "expired token");
    ...
    const replacement = (await changed.json() as any).token;
    ...
    assert.equal((await request("/api/auth/me", serviceToken)).status, 401, "service-bound human session cannot become a direct Panel session");
